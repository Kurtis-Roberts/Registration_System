    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        ...
        <div class="collapse navbar-collapse" id="navbarNavDropdown">
            <ul class="navbar-nav mr-auto">
                <?php echo "Welcome $first $last, Student Advisor"; ?>
            </ul>
            <ul class="navbar-nav">
    ...
    <div class="page-header header container-fluid">
        <div class="container">
            <div class="form-container">
                <h2>Current / Future Courses</h2>
                <p>
                    Here is a list of courses available for students to register for<br>
                </p>
                ...
                $query = "SELECT Course_NUM, Course_Title, Day_Time, Room, Credits, Concat (C.Semester, ' ', C.Course_Year) As Semester, Concat (F.Fname, ' ', F.Lname) as Instructor FROM Course as C Join Faculty F On C.Faculty_ID = F.Faculty_ID  WHERE C.Course_Year >= Year(Curdate()) ORDER BY Course_NUM ASC";

                $result = $conn->query($query);

                if ($result->num_rows > 0) {
                    echo '<table class="table table-striped">';
                    echo '<tr><th>Class</th><th>Description</th><th>Semester</th><th>Day/Time</th><th>Room</th><th>Instructor</th><th>Units</th></tr>';
                    // output data of each row
                    while ($row = $result->fetch_assoc()) {
                        echo "<tbody><tr><td>" . $row["Course_NUM"] . "</td><td>" . $row["Course_Title"] . "</td><td>" . $row["Semester"] . "</td><td>" . $row["Day_Time"] . "</td><td>" . $row["Room"] . "</td><td>" . $row["Instructor"] . "</td><td>" . $row["Credits"] . "</td><td>" . "</td></tr></tbody>";
                    }
                    echo "</table>";
                } else {
